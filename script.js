...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.nav-links a').forEach(link => {
    link.addEventListener('click', e => {
      ...
      // Sempre fecha o menu após clicar
      if (navLinks) {
  ...
  document.querySelectorAll('.hero, .cards, .especializacao, .servicos, .prova, .final')
    .forEach(el => {
      observer.observe(el);
    });
  ...
  if (form) {
    form.addEventListener('submit', e => {
      e.preventDefault();
      alert('Clínica Odonto Exemplo agradece sua avaliação!');
      popup.style.display = 'none';
      form.reset();
    });
  }
